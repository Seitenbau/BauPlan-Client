...
describe('plansReducer', () => {
  ...
  it('returns the state with plans with a default scale of 1', () => {
    const plans = fromJS([{ name: 'Project 1' }]);
    const reducedResult = plansReducer(undefined, {
      ...
      .first()
      .get('scale');
    expect(reducedResult).toEqual(1);
  });
  it('returns the state with plans with the given scale', () => {
    const plans = fromJS([{ name: 'Project 1', scale: 3 }]);
    const reducedResult = plansReducer(undefined, {
      type: REQUEST_PLANS_SUCCESS,
      data: plans
    })
      ...
      .first()
      .get('scale');
    expect(reducedResult).toEqual(3);
  });
